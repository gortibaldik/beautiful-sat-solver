...
import Vue from 'vue'
import results_comm from '@/assets/js/nqueens_results_communication'

export default {
  name: 'N-Queens Results',
  title: 'SAT: N-Queens Results',
  components: {
    mdbRow,
    mdbCol,
    mdbCard,
  ...
  methods: {
    ...
    cumulativeVisualizationCreation(checked) {
      ...
      for (let i = 0; i < checked.length; i++) {
        barChartData.labels.push(`${checked[i]["Algorithm"]} ${checked[i]["Number of Queens"]}`)
        let k = 0
        for (let j = 0; j < this.data.columns.length; j++) {
          if (this.data.columns[j].plotted) {
    ...
    visualizationCreation(checked, nameOfField) {
      ...
      for (let i = 0; i < checked.length; i++) {
        ...
        labelsSet.add(checked[i]["Number of Queens"])
      }
        ...
        for (let lnix = 0; lnix < labelNames.length; lnix++) {
          ...
          for (let cix = 0; cix < checked.length; cix++) {
            if (checked[cix]["Algorithm"] == datasetNames[dix] &&
                checked[cix]["Number of Queens"] == labelNames[lnix]) {
